...
<p>
面向 Lecoo / Emdoor 笔记本的桌面控制中心:实时遥测、EC 风扇模式、电源配置、电池充电限制、键盘背光与机身后部 LED 环 —— 全部通过一个与笔记本嵌入式控制器(EC)通信的 Windows 服务驱动。
</p>
...
<table>
  <tr>
    ...
    <td align="center"><img src="branding/screenshots/fans.png" alt="Fans" width="420"><br><sub>Fans · 自动 / 全速</sub></td>
  </tr>
  <tr>
...
- 🌡️ **风扇模式** —— 自动(EC 内置温控表,跟随激活的电源配置)与全速(两个风扇 100%)。GUI 有意不提供自定义曲线,见下方*已知限制*。
...
- **MSI 尚未代码签名。** 首次运行时 Windows SmartScreen 会提示"未识别的应用程序"——点击*更多信息 → 仍然运行*。GUI 发布带签名版本后会处理。
- **GUI 不提供自定义风扇曲线。** 在客户端评估曲线(如 `0.1.0` / `0.1.1`)意味着每秒多次访问 EC。在这些主板上 EC port-I/O 经常触发系统管理中断(SMI),会短暂阻塞整个 CPU,表现为 DPC 延迟、声音卡顿、游戏掉帧。正确实现需要迟滞、守护进程崩溃保护和分主板调校——属于上游守护进程的职责,我们会在它就绪后接入。在此之前,使用 EC 内置温控表(自动模式)是正确做法。
- **应用内更新检查只会跳转到发布页面**,不会下载也不会安装新 MSI。Tauri 完整 updater 需要签名构建,签名前保留为提示模式。
- **遥测以 0.33 Hz(每 3 秒一次)采样**,窗口被隐藏时完全暂停,以将 EC port-I/O 流量压到最低。
